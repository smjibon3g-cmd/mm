...
function PreviousMonthDetails() {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      setMessId(currentUser['Mess ID']);
    }
  }, []);

  useEffect(() => {
    if (messId) {
      fetch(`/api/months?mess_id=${messId}`)
        .then(res => res.json())
        .then(data => {
          setMonths(data);
          if (data.length > 0) {
      ...
      const fetchMonthData = async () => {
        try {
          const [mealsRes, membersRes, depositsRes, costsRes] = await Promise.all([
            fetch(`/api/meals?mess_id=${messId}&month=${selectedMonth}`),
            fetch(`/api/members?mess_id=${messId}`),
            fetch(`/api/deposits?mess_id=${messId}&month=${selectedMonth}`),
            fetch(`/api/costs?mess_id=${messId}&month=${selectedMonth}`)
          ]);
  ...
  const handleExportExcel = () => {
    ...
    // Deposit Details
    const depositDetailsData = deposits.map(deposit => ({
      Date: deposit.Date,
    ...
    // Meal Cost Details
    const mealCostDetailsData = costs.filter(cost => cost.Category === 'Meal').map(cost => ({
      Date: cost.Date,
    ...
    const individualCostDetailsData = costs.filter(cost => cost.Category === 'Individual').map(cost => ({
      Date: cost.Date,
      Description: cost.Description,
      Amount: cost.Amount,
      'Paid By': memberMap[cost['Paid By Member ID']],
    ...
    const sharedCostDetailsData = costs.filter(cost => cost.Category === 'Shared - Meal Consumers' || cost.Category === 'Shared - All Members').map(cost => ({
      Date: cost.Date,
      Description: cost.Description,
      Amount: cost.Amount,
      'Paid By': memberMap[cost['Paid By Member ID']],
    ...
    XLSX.writeFile(wb, `previous-month-details-${selectedMonth}.xlsx`);
  };
  ...
  const handleDownloadPdf = () => {
    ...
    html2canvas(input, { scrollY: -window.scrollY }).then((canvas) => {
      ...
      pdf.save(`previous-month-details-${selectedMonth}.pdf`);
    });
  };
    ...
    <div className="page" ref={contentRef}>
              ...
              <tr key={date}>
                ...
                {allMemberIds.map(memberId => (
                  <React.Fragment key={memberId + '-meals'}>
                    <td>{mealsByDateAndMember[date][memberId] ? mealsByDateAndMember[date][memberId].breakfast : 0}</td>
                    <td>{mealsByDateAndMember[date][memberId] ? mealsByDateAndMember[date][memberId].lunch : 0}</td>
                    <td>{mealsByDateAndMember[date][memberId] ? mealsByDateAndMember[date][memberId].dinner : 0}</td>
                  </React.Fragment>
                ))}
            </tr>
            ))}
      ...
      <div className="details-section">
        ...
        <table>
          ...
          <tbody>
            {costs.filter(cost => cost.Category === 'Meal').map((cost, index) => (
              <tr key={index}>
      ...
      <div className="details-section">
        ...
        <table>
          ...
          <tbody>
            {costs.filter(cost => cost.Category === 'Individual').map((cost, index) => (
              <tr key={index}>
                <td>{formatDate(cost['Date'])}</td>
                <td>{cost['Description']}</td>
                <td>{cost['Amount']}</td>
      ...
      <div className="details-section">
        ...
        <table>
          ...
          <tbody>
            {costs.filter(cost => cost.Category === 'Shared - Meal Consumers' || cost.Category === 'Shared - All Members').map((cost, index) => (
              <tr key={index}>
                <td>{formatDate(cost['Date'])}</td>
                <td>{cost['Description']}</td>
                <td>{cost['Amount']}</td>
...
export default PreviousMonthDetails;
