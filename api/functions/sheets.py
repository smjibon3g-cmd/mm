...
def update_row(spreadsheet_id, sheet_name, row_index, new_data):
    ...
    for header in headers:
        updated_values.append(new_data.get(header, records[row_index - 2].get(header, '')))
...
def setup_sheets(spreadsheet_id):
    ...
    spreadsheet = gspread.authorize(Credentials.from_service_account_info(json.loads(os.environ["GOOGLE_CREDENTIALS"]), scopes=SCOPE)).open_by_key(spreadsheet_id)
    ...
    sheets_to_create = {
        ...
        "Deposits": ["Deposit ID", "Date", "Member ID", "Amount", "Details", "Mess ID"],
