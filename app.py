...
def api_mark_all_notifications_as_read():
    user_id = request.args.get('user_id')
    print(f"Attempting to mark all notifications as read for user: {user_id}")
    if not user_id:
        print("User ID is required for read_all")
        return jsonify({'error': 'User ID is required'}), 400

    success = mark_all_notifications_as_read(SHEET_ID, user_id)
    if success:
        print(f"Successfully marked all notifications as read for user: {user_id}")
        return jsonify({'success': True})
    else:
        print(f"Failed to mark all notifications as read for user: {user_id}")
